"""
Dense + Reranker standalone — sem BM25, sem CRAG.

Pipeline: Dense search (top-candidates) → Reranker cross-encoder → top_k

Uso:
    python scripts/run_dense_reranker.py --config configs/dense_reranker_bge_m3_ft_v2_ft.yaml
"""
...
def parse_args():
    ...
    p.add_argument("--config", default="configs/dense_reranker_bge_m3_ft_v2_ft.yaml")
...
def main():
    args = parse_args()
    console.rule("[bold]Dense + Reranker[/]")
    ...
    if query_instruction:
        console.print(f"  Instrução de query: {query_instruction}")
    ...
    console.rule(f"[bold]3. Busca densa (top-{cfg['retrieval']['candidates']})[/]")
    ...
    console.rule("[bold]4. Reranking[/]")
    rr = cfg["reranker"]
    final_rank = rerank(
        query_texts, ds["query_ids"], dense_rankings,
    ...
    print_metrics("Dense+Reranker — Métricas (score >= 1)", metrics)

    metrics_strict = {}
    if cfg["evaluation"].get("also_strict") and ds["qrels_strict"]:
        metrics_strict = evaluate_rankings(final_rank, ds["qrels_strict"], k_values=k_values)
        print_metrics("Dense+Reranker — Métricas (score == 2)", metrics_strict)
